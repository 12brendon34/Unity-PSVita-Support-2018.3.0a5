...
typedef struct pthread_storage_t
{
  ...
  char					doStartStopCallbacks;	// Thread will call pthread_add_thread_callback() when started and pthread_delete_thread_callback() when stopped.
  size_t				stackSize;
...
typedef struct pthread_storage_t* pthread_t;

#define PTHREAD_NAME_MAX_LEN	(SCE_UID_NAMELEN - 9)
