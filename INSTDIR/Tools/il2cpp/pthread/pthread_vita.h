...
EXTERN void pthread_set_add_thread_callback_ext(pthread_callback_t callback);
EXTERN void pthread_set_delete_thread_callback_ext(pthread_callback_t callback);

EXTERN void pthread_set_default_priority_ext(int priority);
EXTERN int pthread_get_default_priority_ext();
EXTERN int pthread_get_default_priority_ceiling_ext();

EXTERN void pthread_set_default_stack_size_np(int stackSize);
EXTERN int pthread_get_default_stack_size_np();


EXTERN void pthread_set_default_affinity_ext(unsigned int affinity);
EXTERN unsigned int pthread_get_default_affinity_ext();

EXTERN void pthread_set_default_dostartstopcallbacks_ext(int doStartStopCallbacks);
EXTERN int pthread_get_default_dostartstopcallbacks_ext();

EXTERN int pthread_attr_init(pthread_attr_t *attr);
EXTERN int pthread_attr_destroy(pthread_attr_t *attr);

EXTERN int pthread_attr_setaffinity_ext(pthread_attr_t *attr, unsigned int affinity);
EXTERN int pthread_attr_getaffinity_ext(const pthread_attr_t *attr, unsigned int* affinity);

enum {
	PTHREAD_CALLBACKS_NONE = 0,
	PTHREAD_CALLBACKS_ON_START = (1<<0),
	PTHREAD_CALLBACKS_ON_STOP = (1<<1),
};

EXTERN int pthread_attr_setdostartstopcallbacks_ext(pthread_attr_t *attr, int state);
EXTERN int pthread_attr_getdostartstopcallbacks_ext(pthread_attr_t *attr, int *state);


// Sets the current thread as the pthread mainthread, specifically so pthread_getspecific() works correctly for IL2CPP
EXTERN void pthread_set_self_as_mainthread_np();

// Gets the pthread main thread.
EXTERN pthread_t pthread_get_mainthread_np();

// Sets the behavior for calls to pthread_self from threads which are not a pthread, e.g. native threads.
// If enable = 1 then calls to pthread_self made from a non-pthread thread will return the user-main pthread.
// If enable = 0 then calls to pthread_self made from a non-pthread thread will return PTHREAD_INVALID_THREAD.
// The user-main pthread is either the first pthread that was created or the last pthread which called pthread_set_self_as_mainthread_np().
// Returns the previous state.
EXTERN int pthread_set_redirect_self_to_main_np(int enable);

#define PTHREAD_VITA_NUM_REGISTERS 16

typedef struct _pthread_CpuRegisterInfo{
	SceSize		size;
	SceUInt32	cpsr;
	SceUInt32	reg[PTHREAD_VITA_NUM_REGISTERS];
	SceUInt32	tpidrurw;
	SceUInt32	teehbr;
	SceUInt32	sb;
	SceUInt32	st;
} pthread_CpuRegisterInfo;

typedef struct _pthread_VfpRegisterInfo {
	SceSize		size;
	SceUInt32	fpscr;
	struct {
		SceUInt32	word[4];
	} reg[PTHREAD_VITA_NUM_REGISTERS];
} pthread_VfpRegisterInfo;

EXTERN int pthread_suspend_np(pthread_t id);
EXTERN int pthread_resume_np(pthread_t id);
EXTERN int pthread_get_context_np(pthread_t id, pthread_CpuRegisterInfo *pCpuRegisterInfo, pthread_VfpRegisterInfo *pVfpRegisterInfo);
EXTERN int pthread_set_context_np(pthread_t id, const pthread_CpuRegisterInfo *pCpuRegisterInfo, const pthread_VfpRegisterInfo *pVfpRegisterInfo);
...
// Gets the native SCE thread ID for the specified pthread.
EXTERN SceUID pthread_getnativeid_np(pthread_t thread);

// Gets a pthread given a native SCE thread ID, returns -1 if the SCE thread ID is not a pthread.
EXTERN pthread_t pthread_getfromnativeid_np(SceUID uid);


#define PTHREAD_INVALID_THREAD ((pthread_t)-1)
...
EXTERN int pthread_cond_timedwait_relative_np(pthread_cond_t* cond, pthread_mutex_t* mutex, const struct timespec* spec);
